...
export class TagUtils {
     ...
     */
    static getTagKey(frontmatter: Record<string, unknown>): string {
        if (!frontmatter) return "tags";
        const found = Object.keys(frontmatter).find(
            (k) => k.toLowerCase() === "tags",
        );
        return found ?? "tags";
    }

    /** Converts a raw frontmatter tag value (array, string, or other) into a string list. */
    private static rawTagsToStrings(value: unknown): string[] {
        if (Array.isArray(value)) {
            return (value as unknown[]).map((tag) => String(tag));
        }
        return typeof value === "string" ? [value] : [];
    }

    /**
     * Parses frontmatter YAML text into a mutable mapping.
     * Returns null when the parsed value is not a YAML mapping.
     * Rethrows parse errors for caller-specific handling.
     */
    private static parseFrontmatterMapping(
        text: string,
    ...
    static getExistingTags(
        ...
        const key = TagUtils.getTagKey(frontmatter);
        if (!(key in frontmatter)) return [];

        try {
            return TagUtils.rawTagsToStrings(frontmatter[key]);
        } catch {
    ...
    static formatTag(tag: unknown, format: TagFormat = "kebab-case"): string {
        // Only primitives can form valid tags; objects would stringify to [object Object]
        if (
            tag === null ||
            tag === undefined ||
            (typeof tag !== "string" &&
        ...
        if (formatted.includes("/")) {
            ...
            const formattedParts = parts.map((part) =>
                TagUtils.formatTagPart(part, format),
            );
            return formattedParts.filter((p) => p.length > 0).join("/");
        }

        return TagUtils.formatTagPart(formatted, format);
    }
    ...
    private static formatTagPart(part: string, format: TagFormat): string {
        ...
        switch (format) {
            case "camelCase":
                return TagUtils.toCamelCase(trimmed);
            case "PascalCase":
                return TagUtils.toPascalCase(trimmed);
            case "snake_case":
                return TagUtils.toSnakeCase(trimmed);
            case "original":
            ...
            case "kebab-case":
            default:
                return TagUtils.toKebabCase(trimmed);
        }
    }
    ...
    static async clearTags(app: App, file: TFile): Promise<TagOperationResult> {
        try {
            ...
            if (!frontmatterPosition) {
                return {
                    success: true,
                    message: "Skipped: Note has no frontmatter",
                    tags: [],
                };
            }
            ...
            try {
                frontmatter = TagUtils.parseFrontmatterMapping(frontmatterText);
            } catch (yamlError) {
            ...
            if (!frontmatter) {
                return {
                    success: true,
                    message: "No valid frontmatter",
                    tags: [],
                };
            }

            const tagKey = TagUtils.getTagKey(frontmatter);
            if (!(tagKey in frontmatter)) {
                return { success: true, message: "No tags to clear", tags: [] };
            }

            const tagsToRemove = TagUtils.rawTagsToStrings(frontmatter[tagKey]);
            ...
            if (newContent !== content) {
                try {
                    ...
                    throw new Error(
                        `Failed to modify file: ${modifyError instanceof Error ? modifyError.message : String(modifyError)}`,
                    );
                }
            }
    ...
    static async updateNoteTags(
        ...
        try {
            ...
            const yamlReadyTags = TagUtils.formatTags(
                allTags,
                false,
                tagFormat,
            );
            ...
            try {
                ...
                if (!replaceTags && existingFrontmatter) {
                    const existingTags =
                        TagUtils.getExistingTags(existingFrontmatter);

                    // If we have existing tags and we're not replacing, combine them
                    if (existingTags.length > 0) {
                        const combined = TagUtils.mergeTags(
                            existingTags,
                            yamlReadyTags,
                        );
                    ...
                    const existingSet = new Set(
                        existingTags.map((t) => t.toString().trim()),
                    );
                    const newSet = new Set(
                        yamlReadyTags.map((t) => t.toString().trim()),
                    );
            ...
            try {
                ...
                if (frontmatterPosition) {
                    ...
                    frontmatter[TagUtils.getTagKey(frontmatter)] =
                        yamlReadyTags;
                ...
                debugLog(
                    "<<< updateNoteTags: failed to update frontmatter",
                    updateError,
                );
                throw new Error(
                    `Failed to update frontmatter: ${updateError instanceof Error ? updateError.message : String(updateError)}`,
                );
            }
    ...
    static getAllTagsFromFrontmatter(app: App): string[] {
        ...
        app.vault.getMarkdownFiles().forEach((file) => {
            ...
            const tagKey = TagUtils.getTagKey(cache.frontmatter);
            if (cache.frontmatter[tagKey] != null) {
                debugLog("getAllTagsFromFrontmatter:tags:1/2:", tags);
                TagUtils.getExistingTags(cache.frontmatter).forEach((tag) =>
                    tags.add(tag),
                );
    ...
    static getAllTags(app: App): string[] {
        return TagUtils.getAllTagsFromFrontmatter(app);
    }
    ...
    static async saveAllTags(app: App, tagDir: string = "tags"): Promise<void> {
        const tags = TagUtils.getAllTagsFromFrontmatter(app);
        ...
        // Vault paths are always forward-slash separated; do not use node 'path' (not mobile-safe)
        const filePath = [folderPath.replace(/\/+$/, ""), `tags_${dateStr}.md`]
            .filter(Boolean)
            .join("/");
        ...
        const saveTags = async (): Promise<void> => {
            try {
                ...
                if (!file) {
                    ...
                    throw new TagError(
                        `Cannot save tags: ${filePath} exists but is not a file`,
                    );
                }
        ...
        const modal = new ConfirmationModal(
            ...
            () => {
                // Errors are surfaced via Notice inside saveTags
                void saveTags();
            },
        );
     ...
     */
    static async getTagsFromFile(
        app: App,
        filePath: string,
        format: TagFormat = "kebab-case",
    ): Promise<string[] | null> {
        try {
            if (!filePath) {
                return null;
            ...
            return content
                ...
                .filter(Boolean)
                .map((tag) => TagUtils.formatTag(tag, format));
        } catch {
    ...
    static formatTags(
        ...
        const result = tags
            ...
            .map((tag) => {
                try {
                    const formatted = TagUtils.formatTag(tag, format);
                    if (formatted !== tag) {
                        debugLog(
                            `formatTag transformed: "${String(tag)}" -> "${formatted}"`,
                        );
                    }
    ...
    static async writeTagsToFrontmatter(
        ...
        try {
            ...
            const formattedTags = TagUtils.formatTags(tags, false, tagFormat);
            ...
            if (replace) {
                ...
                const existingTags =
                    cache && cache.frontmatter
                        ? TagUtils.getExistingTags(cache.frontmatter)
                        : [];
                finalTags = TagUtils.mergeTags(existingTags, formattedTags);
            }
            ...
            if (frontmatterPosition) {
                try {
                    ...
                    try {
                        frontmatter =
                            TagUtils.parseFrontmatterMapping(frontmatterText);
                    } catch (yamlError) {
                        //console.error('YAML parse error:', yamlError);
                        throw new Error(
                            `YAML parse error: ${yamlError instanceof Error ? yamlError.message : String(yamlError)}`,
                        );
                    }
                    if (!frontmatter) {
    ...
    static isFileExcluded(
        ...
        for (const pattern of excludePatterns) {
            try {
                ...
                if (TagUtils.matchesGlobPattern(filePath, pattern)) {
                    return true;
                }
    ...
    private static getMarkdownFilesFromFolder(folder: TFolder): TFile[] {
        ...
        for (const child of folder.children) {
            if (child instanceof TFile && child.extension === "md") {
                ...
                markdownFiles.push(
                    ...TagUtils.getMarkdownFilesFromFolder(child),
                );
            }
        }
    ...
    static getNonExcludedMarkdownFiles(
        ...
        if (folder) {
            ...
            allFiles = TagUtils.getMarkdownFilesFromFolder(folder);
        } else {
        ...
        return allFiles.filter(
            (file) => !TagUtils.isFileExcluded(file, excludePatterns),
        );
    }
    ...
    static async flattenHierarchicalTags(
        ...
        try {
            ...
            const hasHierarchicalTags = existingTags.some((tag) =>
                tag.includes("/"),
            );
            if (!hasHierarchicalTags) {
                return {
                    success: true,
                    message: "No hierarchical tags found",
                    tags: existingTags,
                };
            }
            ...
            for (const tag of existingTags) {
                if (tag.includes("/")) {
                    ...
                    for (const part of parts) {
                        flattenedTags.add(TagUtils.formatTag(part, tagFormat));
                    }
                } else {
                    flattenedTags.add(TagUtils.formatTag(tag, tagFormat));
                }
            }
    ...
    static async renameTagInVault(
        ...
        const normalizedOldTag = oldTag.startsWith("#")
            ? oldTag.substring(1).toLowerCase()
            : oldTag.toLowerCase();
        const formattedNewTag = TagUtils.formatTag(newTag, tagFormat);

        if (!normalizedOldTag || !formattedNewTag) {
            return {
                success: false,
                message: "Invalid tag names",
                affectedFiles: 0,
            };
        }
        ...
        for (const file of markdownFiles) {
            try {
                ...
                const newTags = existingTags.map((tag, idx) => {
                    ...
                    return TagUtils.formatTag(tag, tagFormat);
                });
                ...
                if (!frontmatter) {
                    // The outer catch counts this file as an error, matching
                    // the old behavior where writing to a non-mapping failed
                    throw new Error("Invalid frontmatter format");
                }
