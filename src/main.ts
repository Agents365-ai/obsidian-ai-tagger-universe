...
export default class AITaggerPlugin extends Plugin {
    ...
    public async loadSettings(): Promise<void> {
        // Persisted settings are this plugin's own data.json; legacy fields are migrated below
        const oldSettings = (await this.loadData()) as
            | (Omit<Partial<AITaggerSettings>, "serviceType"> & {
                  serviceType?: "local" | "cloud" | "ollama";
    ...
    private async initializeLLMService(): Promise<void> {
        ...
        this.llmService =
            this.settings.serviceType === "local"
                ? new LocalLLMService(
                      {
                          ...
                          llmTemperatureOverride:
                              this.settings.llmTemperatureOverride,
    ...
    public async onload(): Promise<void> {
        ...
        this.registerView(
            ...
            (leaf) =>
                new TagNetworkView(
                    leaf,
                    this.tagNetworkManager.getNetworkData(),
                    this.app,
                    this.t,
        ...
        this.registerView(
            TAG_ANALYTICS_VIEW_TYPE,
            (leaf) =>
                new TagAnalyticsView(
                    leaf,
                    this.app,
                    this.t,
                    this.tagAnalyticsManager,
                ),
        );
        ...
        this.addRibbonIcon(
            ...
            () => {
                this.analyzeAndTagCurrentNote().catch(console.error);
            },
        );

        this.addRibbonIcon("git-graph", this.t.messages.viewTagNetwork, () => {
            this.showTagNetwork().catch(console.error);
        });
    }

    public onunload(): void {
        // Clean up resources; active requests are cancelled asynchronously
        void this.llmService?.dispose().catch(console.error);
    ...
    public async showTagNetwork(): Promise<void> {
        try {
            const statusNotice = new Notice(
                this.t.messages.buildingTagNetwork,
                0,
            );
    ...
    public getNonExcludedMarkdownFiles(): TFile[] {
        return TagUtils.getNonExcludedMarkdownFiles(
            this.app,
    ...
    public async clearDirectoryTags(
        directory: TFile[],
    ): Promise<BatchProcessResult> {
        return this.tagOperations.clearDirectoryTags(directory);
    }

    public handleTagUpdateResult(
        result: TagOperationResult | null | undefined,
        silent = false,
    ): void {
        if (!result) {
            if (!silent)
                new Notice("Failed to update tags: No result returned", 3000);
            return;
        }
        ...
        if (result.success) {
            ...
            if (!silent)
                new Notice(
                    `Failed to update tags: ${result.message || "Unknown error"}`,
                    4000,
                );
    ...
    public async analyzeAndTagFiles(files: TFile[]): Promise<void> {
        ...
        try {
            ...
            new Notice(
                `Successfully tagged ${successful} out of ${files.length} files`,
                4000,
            );
        } catch {
    ...
    public async analyzeAndTagCurrentNote(): Promise<void> {
        ...
        try {
            ...
            new Notice(
                "Failed to analyze note. Please check console for details.",
                4000,
            );
        }
    }
     ...
     */
    public async analyzeWithHybridMode(
        content: string,
    ): Promise<{ tags: string[] }> {
        // Get predefined tags list
        let predefinedTags: string[] = [];
        if (this.settings.tagSourceType === "file") {
            const fileTags = await TagUtils.getTagsFromFile(
                this.app,
                this.settings.predefinedTagsPath,
        ...
        const normalizedGeneratedTags = TagUtils.formatTags(
            hybridResult.suggestedTags || [],
        );
        const normalizedMatchedTags = TagUtils.formatTags(
            hybridResult.matchedExistingTags || [],
        );

        // Use TagUtils.mergeTags to combine and deduplicate
        const allTags = TagUtils.mergeTags(
            normalizedGeneratedTags,
            normalizedMatchedTags,
        );
                ...
                switch (this.settings.taggingMode) {
                    ...
                    case TaggingMode.PredefinedTags: {
                        ...
                        const predefinedTags =
                            this.settings.tagSourceType === "file"
                                ? (await TagUtils.getTagsFromFile(
                                      this.app,
                                      this.settings.predefinedTagsPath,
                                      this.settings.tagFormat,
                                  )) || []
                    ...
                    default:
                        ...
                        return {
                            success: false,
                            message: `Unsupported tagging mode: ${String(
